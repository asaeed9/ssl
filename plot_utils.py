...
def evaluate(model, Xs, ys, ytrue, lbl):
...
def evaluate_and_plot(model, Xs, ys, ytrue, lbl, subplot=None, block=False):
    ...
    print("Module>> plotutils, Function>>evaluate_and_plot,Xs ALL>>{} ,Xs[:, 0]>>{}, Xs[:, 1]>>{}".format(Xs, Xs[:, 0],
                                                                                                          Xs[:, 1]))
    ...
    try:
        if hasattr(model, 'predict_from_probabilities') and model.predict_from_probabilities:
            plt.contour((probas[:, 0] < np.average(probas[:, 0])).reshape((gridsize, gridsize)),
                        extent=(minx, maxx, miny, maxy), origin='lower')
        else:
            plt.contour(model.predict(Xfull).reshape((gridsize, gridsize)), extent=(minx, maxx, miny, maxy),
                        origin='lower')
    except:
